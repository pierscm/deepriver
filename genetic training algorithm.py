...
class our_bot(BasePokerPlayer):
    ...
    def __init__(self, aggressiveness_raise_prob_factor=1, frequency_call_factor=1, raise_percent=1, raise_prob=1,
                 name='ourbot'):
    ...
    def model_dict(self, round_state):
        ...
        for x in self.opponent_model.keys():
            try:
                self.opponent_model[x]['frequency'] = (self.opponent_model[x]['call'] + self.opponent_model[x][
                    'raise']) / (self.opponent_model[x]['fold'] + self.opponent_model[x]['raise'] +
                                 self.opponent_model[x]['call'])
            except:
                self.opponent_model[x]['frequency'] = 0
            try:
                self.opponent_model[x]['aggressiveness'] = self.opponent_model[x]['raise_shares'] / (
            ...
            if self.opponent_model[x]['raise_this_round'] > 0:
                self.opponent_model[x]['probability'] += (1 - self.opponent_model[x][
                    'aggressiveness'] * self.aggressiveness_raise_prob_factor) \
    ...
    def declare_action(self, valid_actions, hole_card, round_state):
        ...
        if (win_rate >= max_of_list * self.raise_prob) and (valid_actions[2]['amount']['max'] != -1):
            action = valid_actions[2]  # fetch raise action info
            bet = random.uniform(action['amount']['min'],
                                 action['amount']['min'] * (action['amount']['min'] * self.raise_percent))
...
for world in range(2):
    GameNumber = 0
    print("W BEGIN")
    config = setup_config(max_round=5, initial_stack=10000, small_blind_amount=20)
    ...
    agg = [(random.randint(5, 300) / 100), (random.randint(5, 300) / 100), (random.randint(5, 300) / 100),
           (random.randint(5, 300) / 100)]
    freq = [(random.randint(5, 300) / 100), (random.randint(5, 300) / 100), (random.randint(5, 300) / 100),
            (random.randint(5, 300) / 100)]
    rand_raise = [(random.randint(5, 300) / 100), (random.randint(5, 300) / 100), (random.randint(5, 300) / 100),
                  (random.randint(5, 300) / 100)]
    raise_prob = [(random.randint(80, 150) / 100), (random.randint(80, 150) / 100), (random.randint(80, 150) / 100),
                  (random.randint(80, 150) / 100)]
    ...
    config.register_player(name="ourbot1",
                           algorithm=our_bot(aggressiveness_raise_prob_factor=agg[0], frequency_call_factor=freq[0],
                                             raise_percent=rand_raise[0], raise_prob=raise_prob[0], name="ourbot1"))
    config.register_player(name="ourbot2",
                           algorithm=our_bot(aggressiveness_raise_prob_factor=agg[1], frequency_call_factor=freq[1],
                                             raise_percent=rand_raise[1], raise_prob=raise_prob[1], name="ourbot2"))
    config.register_player(name="ourbot3",
                           algorithm=our_bot(aggressiveness_raise_prob_factor=agg[2], frequency_call_factor=freq[2],
                                             raise_percent=rand_raise[2], raise_prob=raise_prob[2], name="ourbot3"))
    config.register_player(name="ourbot4",
                           algorithm=our_bot(aggressiveness_raise_prob_factor=agg[3], frequency_call_factor=freq[3],
                                             raise_percent=rand_raise[3], raise_prob=raise_prob[3], name="ourbot4"))
    ...
    game_resulta['agg_start'] = game_resulta['aggressiveness_raise_prob_factor']
    game_resulta['freq_start'] = game_resulta['frequency_call_factor']
    game_resulta['raise_perc_start'] = game_resulta['raise_percent']
    game_resulta['raise_prob_start'] = game_resulta['raise_prob']

    game_resulta['ours'] = game_resulta['name'].apply(lambda x: True if x[:3] == 'our' else False)
    ourbot_df = game_resulta[game_resulta.ours == True]
    ourbot_df = ourbot_df.sort_values('stack')
    ourbot_df = ourbot_df[:2]
    ourbot_df_stack = ourbot_df['stack'].sum()
    ourbot_df['win_factor'] = ourbot_df['stack'] / ourbot_df_stack
    ourbot_df['agg_fact'] = ourbot_df['aggressiveness_raise_prob_factor'] * ourbot_df['win_factor']
    ourbot_df['freq_fact'] = ourbot_df['frequency_call_factor'] * ourbot_df['win_factor']
    ourbot_df['raise_percent'] = ourbot_df['raise_percent'] * ourbot_df['win_factor']
    ourbot_df['raise_prob'] = ourbot_df['raise_prob'] * ourbot_df['win_factor']
    agg_fact = ourbot_df['agg_fact'].sum()
    freq_fact = ourbot_df['freq_fact'].sum()
    raise_perc = ourbot_df['raise_percent'].sum()
    raise_probab = ourbot_df['raise_prob'].sum()

    game_resulta['aggressiveness_raise_prob_factor'] = agg_fact
    game_resulta['frequency_call_factor'] = freq_fact
    game_resulta['raise_percent'] = raise_perc
    game_resulta['raise_prob'] = raise_probab
    game_resulta['game'] = GameNumber
    game_resulta['world'] = world
    print(game_resulta)
    ...
    for evolution in range(2):
        ...
        freq_factor[1] = freq_fact
        raise_percent[1] = raise_perc
        raise_prob[1] = raise_probab
        ...
        game_resultb['aggressiveness_raise_prob_factor'] = pd.Series(
            [0, 0, 0, 0, agg_factor[1], agg_factor[2], agg_factor[3], agg_factor[4]])
        game_resultb['frequency_call_factor'] = pd.Series(
            [0, 0, 0, 0, freq_factor[1], freq_factor[2], freq_factor[3], freq_factor[4]])
        ...
        game_resultb['agg_start'] = game_resultb['aggressiveness_raise_prob_factor']
        game_resultb['freq_start'] = game_resultb['frequency_call_factor']
        game_resultb['raise_perc_start'] = game_resultb['raise_percent']
        game_resultb['raise_prob_start'] = game_resultb['raise_prob']

        game_resultb['ours'] = game_resultb['name'].apply(lambda x: True if x[:3] == 'our' else False)
        ourbot_df = game_resultb[game_resultb.ours == True]
        ourbot_df = ourbot_df.sort_values('stack')
        ourbot_df = ourbot_df[:2]
        ourbot_df_stack = ourbot_df['stack'].sum()
        ourbot_df['win_factor'] = ourbot_df['stack']/ourbot_df_stack
        ...
        game_resultb['aggressiveness_raise_prob_factor'] = agg_fact
        game_resultb['frequency_call_factor'] = freq_fact
        game_resultb['raise_percent'] = raise_perc
        game_resultb['raise_prob'] = raise_probab
        game_resultb['game'] = GameNumber
        game_resultb['world'] = world
        print("finished game.")
